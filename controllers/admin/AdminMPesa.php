...
class AdminMPesaController extends ModuleAdminController
{
    public function __construct()
    {
        $this->bootstrap = true;
        $this->table = 'osenmpesa_system';
        $this->className = 'MPesa';
        $this->lang = true;
        ...
        $this->fields_list = array(
            'id_osenmpesa_system' => array('title' => $this->l('ID'), 'align' => 'center', 'width' => 30),
            'logo' => array(
                'title' => $this->l('Logo'),
                'align' => 'center',
                'image' => 'pay',
    ...
    public function renderForm()
    {
        ...
        $this->fields_form = array(
            ...
            'input' => array(
                ...
                array(
                    ...
                    'options' => array(
                        'query' => array(
                            array('id' => MPesa::CART_BOTH, 'name' => $this->l('Real and virtual')),
                            array('id' => MPesa::CART_REAL, 'name' => $this->l('Real')),
                            array('id' => MPesa::CART_VIRTUAL, 'name' => $this->l('Virtual')),
                        ),
                        'id' => 'id',
                        'name' => 'name'
                    )
        ...
        $osenmpesa_system_carrier_ids = $obj->getCarriers();
        ...
        foreach ($carriers as $carrier) {
            $this->fields_value['carrierBox_' . $carrier['id_carrier']] = Tools::getValue('carrierBox_' . $carrier['id_carrier'],
                (in_array($carrier['id_carrier'], $osenmpesa_system_carrier_ids)));
        }

        $osenmpesa_system_group_ids = $obj->getGroups();
        ...
        if (Group::isFeatureActive()) {
            ...
            foreach ($groups as $group) {
                $this->fields_value['groupBox_' . $group['id_group']] = Tools::getValue('groupBox_' . $group['id_group'],
                    (in_array($group['id_group'], $osenmpesa_system_group_ids)));
            }
        }
